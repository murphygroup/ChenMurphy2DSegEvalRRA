...
if __name__ == '__main__':
	merged_list = ['DeepCell 0.12.3 cell membrane 95% after merged', 'DeepCell 0.12.3 cell membrane 90% after merged', 'DeepCell 0.12.3 cell membrane 75% after merged', 'DeepCell 0.12.3 cell membrane 60% after merged']
	...
	for mask_dir in mask_dir_list:
		...
		for i in [0.0, 0.2, 0.8]:
			...
			if i == 0.0:
				with open(join(os.path.dirname(mask_dir), 'repaired_mask', 'metrics_' + 'cell_matched_mask_deepcell_membrane-0.12.3.pickle_v1.8.json'), 'r') as f:
					metrics = json.load(f)
			else:
	...
	ax.legend()
	fig.savefig('/home/hrchen/Documents/Research/hubmap/fig/manuscript_v1.8/merged_metrics.png', dpi=500, bbox_inches='tight')
	plt.clf()
	# plt.show()
	# plt.close()
