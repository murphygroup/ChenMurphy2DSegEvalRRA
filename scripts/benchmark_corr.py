...
if __name__ == '__main__':
	...
	# # if not os.path.exists(join(file_dir, 'data', 'output', 'seg_score_corr_seg.csv')):
	# if True:
	...
	#
	# 				f = open(join(data_dir, tile, 'metrics_' + method + '_v27.json'))
	# 				metrics = json.load(f)
	...
	#
	# 	seg_score_dataframe.to_csv(join(output_dir, 'seg_score_corr_seg.csv'))
	#
	# 	pearson_list.append(pearsonr(seg_score_dataframe['seg_score'], seg_score_dataframe['quality_score'])[0])
	#
	# else:
	# 	seg_score_dataframe = pd.read_csv(join(output_dir, 'seg_score_corr_seg.csv'), index_col=0)
	#
	# seg_score_dataframe = seg_score_dataframe.drop(seg_score_dataframe.loc[seg_score_dataframe['seg_score'] == 1.0].index)
	# # seg_score_dataframe = seg_score_dataframe.reindex(methods_abre_list)
	# # print(seg_score_dataframe)
	...
	# if True:
		...
		f1_score_dataframe.to_csv(join(output_dir, 'f1_score', 'f1_score_corr_expert_' + str(round(jaccard, 2)) + '.csv'))





	f1_score_dir_list = sorted(glob.glob(join(file_dir, 'data', 'output', 'f1_score', 'f1_score_corr_*.csv')))
	...
	f1_score_dataframe = pd.read_csv(join(file_dir, 'data', 'output', 'f1_score', 'f1_score_corr_expert_0.3.csv'), index_col=0)
