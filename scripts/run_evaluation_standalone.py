...
def get_image_channels(img_dir):
	channel_dirs = glob.glob(join(os.path.dirname(img_dir), 'channels', '*tif'))
	channel_pieces = []
...
def get_evaluation(img_dir, mask_dir):
	...
	# PCA_model = pickle.load(open(join('/home/hrchen/Documents/Research/hubmap/github_lab/CellSegmentationEvaluator/pipeline/evaluation', 'pca.pickle'), "rb"))
	PCA_model = pickle.load(open(join('/Users/hrchen/Downloads/segmentation_RRA/data/output/pca.pickle'), "rb"))
...
if __name__ == '__main__':
	img_dir = sys.argv[1]
	mask_dir = sys.argv[2]
	# img_dir = '/data/HuBMAP/CODEX/HBM244.TJLK.223/R001_X004_Y004/random_gaussian_0'
	# mask_dir = '/data/HuBMAP/CODEX/HBM244.TJLK.223/R001_X004_Y004/random_gaussian_0/shifted_mask'
	original_masks = sorted(glob.glob(join(mask_dir, 'cell_matched_mask*.pickle')))
	...
	for i in range(0, len(cell_matched_masks)):
		if not os.path.exists(join(mask_dir, 'metrics_' + os.path.basename(cell_matched_masks[i]) + '_v28.json')):
		# if True:
			print(cell_matched_masks[i])
			
			metrics = get_evaluation(img_dir, [original_masks[i], cell_matched_masks[i], nuclear_matched_mask[i], cell_outside_nucleus_matched_mask[i]])
			# save QC json file
				
			with open(join(mask_dir, 'metrics_' + os.path.basename(cell_matched_masks[i]) + '_v28.json'), 'w') as f:
				json.dump(metrics, f)
			# except:
			# 	pass
